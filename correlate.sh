...
# Temporarily remove logs for debugging
#rm "$dir"/*.log
...
awk -W interactive '{split(substr($2,11),a,":"); printf a[1]" "} {for(i=1;i<=NF;i++) {if($i~/^pid=/ || $i~/^exe=/) {printf "%s",substr($i,5)" "} if($i~/^euid=/) {printf "%s",substr($i,6)" "} if($i~/^laddr=/) {printf "%s",substr($i,7)" "}}} {print ""}' > "$dir"/conn.fifo &
# The awk program above is made of 3 main blocks:
...
awk -W interactive '{split(substr($2,11),a,":"); printf a[1]" "} {for(i=1;i<=NF;i++) {if($i~/^pid=/ || $i~/^exe=/) {printf "%s",substr($i,5)" "}}} {print ""}' > "$dir"/close.fifo &
# The awk program above is made of 3 main blocks:
